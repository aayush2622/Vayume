    ...
    in
    {
      virtualisation.vmVariant = {
        ...
        services.displayManager.sddm.settings.General.GreeterEnvironment =
          lib.mkForce "XCURSOR_THEME=${config.vayume.theme.cursorTheme},XCURSOR_SIZE=${toString config.vayume.theme.cursorSize},XCURSOR_PATH=${config.vayume.theme.cursorPackage}/share/icons,QT_QUICK_BACKEND=software";
    ...
    let
      mkVmApp =
        ...
        in
        {
          ...
          program = lib.getExe (
            pkgs.writeShellScriptBin "vayume-vm-${hostName}" ''
              set -eu
