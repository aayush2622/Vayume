{ lib, config, ... }: {
  ...
  options.flake.vayumeLib.desktopActions = lib.mkOption {
    ...
    default =
      let
        choice = id: label: app: desktop: argv: {
          inherit
            id
            label
            app
      ...
      in
      {
        ...
        editor = {
          ...
          choices = [
            ...
            (choice "android-studio" "Android Studio" "AndroidStudio" "android-studio.desktop" [
              "android-studio"
            ])
          ];
        };
        ...
        browserReload.choices = [
          (choice "zen-reload" "Zen reload" "ZenBrowser" null [
            "vayume"
            "zen-reload"
          ])
        ];
        systemMonitor.choices = [
          (choice "btop" "btop" "Terminal" null [
            "kitty"
            "-e"
            "btop"
          ])
        ...
        colorPicker.choices = [
          (choice "hyprpicker" "hyprpicker" null null [
            "hyprpicker"
            "-a"
          ])
        ];
  ...
  config.flake.vayumeLib.mkDesktopActions =
    pkgs:
    lib.mapAttrs (
      name: action:
      let
        ...
        installed =
          argv:
          "command -v ${lib.escapeShellArg (builtins.head argv)} >/dev/null 2>&1${
            lib.optionalString (
              builtins.head argv == "vayume"
            ) " && vayume --has ${lib.escapeShellArg (builtins.elemAt argv 1)}"
          }";
      in
      [
        (lib.getExe (
          pkgs.writeShellScriptBin "vayume-launch-${name}" ''
            preferred=""
            ...
            ${lib.concatMapStrings (c: ''
              if ${installed c.argv}; then exec ${lib.escapeShellArgs c.argv} "$@"; fi
            '') action.choices}
            exec ${pkgs.libnotify}/bin/notify-send -a Vayume "Nothing installed for ${name}" ${
              lib.escapeShellArg (
                if apps == [ ] then
                  "None of: ${lib.concatMapStringsSep ", " (c: builtins.head c.argv) action.choices}"
                else
                  "Enable one of ${
                    lib.concatMapStringsSep ", " (a: "vayume.apps.${a}") apps
                  } in _config.nix, then rebuild."
              )
            }
          ''
        ))
