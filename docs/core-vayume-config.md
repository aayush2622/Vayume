[Index](CONFIGURATION.md)

---

How a DMS toggle turns into a line changed in the real `Host.nix` - the backend both the `vayumeSettings` DMS plugin and a terminal share, so there is exactly one place that understands the file's shape.

## `modules/core/VayumeConfig.nix`

**The problem this solves**: DMS plugins are Wayland-session state, not
the repo. If "Vayume Settings" kept its own JSON database of which apps
are enabled, that database and `Host.nix` would drift the moment either
one changed without the other - two sources of truth pretending to be
one. `vayume-config` is a thin CLI that reads and writes the *real*
`modules/hosts/<host>/Host.nix`, so the repository stays the only
authority. The DMS plugin never parses or writes Nix itself; it just
runs this CLI as a subprocess and reads JSON back, same as a person
would from a terminal.

### Commands

```
vayume-config repo                          # {path, branch, dirty, hostFile}
vayume-config apps list                      # every vayume.apps.* + its state
vayume-config apps set <Name> <true|false> [--if-unmodified-since <epoch>]
vayume-config validate                       # re-evaluate Host.nix, pass/fail
```

`vayume-rebuild` and `vayume-gc` (bare commands, `home.packages`) exist
alongside it - thin wrappers that just `exec sudo -n` the real,
content-addressed script [Dms.nix](desktop-dms.md) registers under the
sudoers `NOPASSWD` rule. They exist because that underlying store path
changes hash on every rebuild; without a stable bare-name wrapper,
anything wanting to trigger a rebuild (a person, or the DMS plugin)
would have to go find the current hash first. The wrapper's own path is
unprivileged - it doesn't touch what the sudoers rule actually permits,
it just always execs the currently-correct target.

### Repo discovery is shared, not re-guessed per script

Both `vayume-rebuild` and `vayume-config` need to answer "where is this
flake checked out?" without a hardcoded `/home/<someone>/vayume`, since
that breaks for every user but the repo's own author. The candidate
list - `~/vayume`, `~/dotfiles`, `~/.dotfiles`, `/etc/nixos` - lives in
exactly one place, [`modules/lib/RepoDiscovery.nix`](../modules/lib/RepoDiscovery.nix)'s
`flake.vayumeLib.repoDiscovery`, and both scripts interpolate that same
Nix-level list into their own shell loop at build time. Change the list
once, both tools pick it up. `vayume-config` doesn't need
`vayume-rebuild`'s `$SUDO_USER` indirection (it never runs under sudo),
just `$HOME` directly.

If no candidate has a `flake.nix`, both fail loudly and stop - never a
silent fallback to some unrelated directory.

### Why a hand-rolled `awk` parser instead of a real Nix parser

`vayume.apps = { ... };` in `Host.nix` is a format this repo itself
generates and controls - one line per app,
`Name.enable = true;`/`false;`, nothing nested inside it. A general Nix
parser to edit that would be solving a much bigger problem than exists;
a small state machine is enough, and it's easy to read in full. The
`awk` script (`vayume-config-apps.awk`, generated by
`VayumeConfig.nix`) walks the whole file with a three-state machine -
*before* the block, *inside* it, *after* it, for good - so a later,
unrelated `foo.enable = true;` elsewhere in `Host.nix` (there are
several: `hardware.bluetooth`, `services.pipewire.alsa`, ...) is never
mistaken for one of `vayume.apps`'s own entries once the block has
closed. An earlier version tracked only a running brace-depth counter
with no "after" state and picked up exactly those false positives -
worth remembering if this ever needs extending, since brace-counting
alone is not enough.

Toggling an app that's already listed rewrites its one line in place,
preserving every other line byte-for-byte. Toggling one that was never
listed (a module added since `Host.nix` was last touched) inserts a new
line just before the block's closing `};`, at the same indentation as
its siblings, rather than guessing at a category comment to attach it
to.

### Atomic writes, validated before they're trusted

`apps set` never truncates `Host.nix` in place. It writes the new
content to a temp file, copies the current file to `Host.nix.bak`, then
atomically renames the temp file over the real one - that rename is the
only moment the file actually changes. It then asks Nix to evaluate
`config.vayume.apps` for real (a real module evaluation, not a syntax
check) against that new content. If evaluation fails, `Host.nix.bak` is
renamed straight back over the broken file and the real Nix error is
printed - the previous, working configuration is never left broken, and
there is exactly one rolling backup, not an accumulating pile. A
`--if-unmodified-since <mtime>` guard lets a caller (the DMS plugin,
having loaded the file at some point) refuse to write over a file that
changed since it was last read, rather than silently clobbering a
manual edit made while a toggle was in flight.

### What "live" actually means here

Writing to `Host.nix` is instant and always safe (validated before it's
kept). Nothing about the *running* system changes until `vayume-rebuild`
actually runs - the DMS plugin says as much: every app toggle is a
config change now, a system change only after the next rebuild. There's
no such thing as an app enable/disable that applies without a rebuild;
unlike wallpaper or some DMS-native appearance settings (DMS's own state,
never routed through this repo at all), installing or removing a
package is never something that happens without one.

---

[← Theme.nix](core-theme.md) · [Index](CONFIGURATION.md) · [DevLanguages.nix →](core-devlanguages.md)
